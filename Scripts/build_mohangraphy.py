...
def generate_html():
    ...
    <head>
        ...
        <style>
            ...
            .submenu a, .nested-header {{ color: #666; padding: 10px 20px; text-decoration: none; font-size: 11px; letter-spacing: 2px; text-transform: uppercase; text-align: center; display: block; transition: 0.2s; }}
    ...
    for m_cat in main_cats:
        ...
        nav_action = f"showSection('{m_cat}')" if any(any(ps) for s in gallery_data[m_cat].values() for ps in s.values()) else "goHome()"
        ...
        if has_submenu:
            ...
            for s_cat, p_dict in gallery_data[m_cat].items():
                ...
                # SPECIAL LOGIC FOR PLACES: Keep deep hierarchy
                if m_cat == "Places" and s_cat != "General":
                    ...
                    nav_html += '</div>'
                # FOR EVERYTHING ELSE (NATURE, BIRDS, etc.): Just show sub-category (Landscape)
                else:
                    if s_cat != "General":
    ...
    print("✅ Build Complete: Places has hierarchy, Nature is flat, Zero Place Labels.")
